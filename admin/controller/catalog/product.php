...
class ControllerCatalogProduct extends Controller {
	...
	protected function getList() {
		...
		$data['href_exportProductData'] = $this->url->link('catalog/product/exportProductData', 'user_token=' . $this->session->data['user_token'] .$url, true);
		$data['href_exportProductImage'] = $this->url->link('catalog/product/exportProductImage', 'user_token=' . $this->session->data['user_token'] .$url, true);
		$data['href_export'] = $this->url->link('catalog/product/exportProductData', 'user_token=' . $this->session->data['user_token'] .$url, true);
	...
	public function exportProductData(){
		...
		$header = array('Product ID','Name','Image','Description','Model','Quantity','Price','Stock Status ID','Stock Status','Manufacturer ID','Manufacturer Name','Related Product','Status','Viewed','Categorie','Tax Class Id','Tax Class','SKU','UPC','EAN','jAN','ISBN','MPN','Location','Shipping','Points','Weight','Weight Class Id','Weight Class','Length','Width','Height','Length Class Id','Length Class','Subtract','Minimum','Sort Order','Tag','Meta Title','Meta Description','Meta Keyword','Date Added','Date Modified','Date Available','Attribute Id','Attribute Name','Attribute Value');

		

		$header = array();

		if ($this->config->get('module_export_product_id_status')) {
			$header[]= 'Product Id';
		}
		if ($this->config->get('module_export_product_name_status')) {
			$header[]= 'Name';
		}
		if ($this->config->get('module_export_image_status')) {
			$header[]= 'Image';
		}
		if ($this->config->get('module_export_description_status')) {
			$header[]= 'Description';
		}
		if ($this->config->get('module_export_model_status')) {
			$header[]= 'Model';
		}
		if ($this->config->get('module_export_quantity_status')) {
			$header[]= 'Quantity';
		}
		if ($this->config->get('module_export_price_status')) {
			$header[]= 'Price';
		}
		if ($this->config->get('module_export_stock_status_id_status')) {
			$header[]= 'Stock Status ID';
		}
		if ($this->config->get('module_export_stock_status_status')) {
			$header[]= 'Stock Status';
		}
		if ($this->config->get('module_export_manufacturer_id_status')) {
			$header[]= 'Manufacturer ID';
		}
		if ($this->config->get('module_export_manufaturer_name_status')) {
			$header[]= 'Manufacturer Name';
		}
		if ($this->config->get('module_export_related_product_status')) {
			$header[]= 'Related Product';
		}
		if ($this->config->get('module_export_product_status_status')) {
			$header[]= 'Status';
		}
		if ($this->config->get('module_export_viewed_status')) {
			$header[]= 'Viewed';
		}
		if ($this->config->get('module_export_category_status')) {
			$header[]= 'Categorie';
		}
		if ($this->config->get('module_export_tax_class_id_status')) {
			$header[]= 'Tax Class Id';
		}
		if ($this->config->get('module_export_tax_class_status')) {
			$header[]= 'Tax Class';
		}
		if ($this->config->get('module_export_sku_status')) {
			$header[]= 'SKU';
		}
		if ($this->config->get('module_export_upc_status')) {
			$header[]= 'UPC';
		}
		if ($this->config->get('module_export_ean_status')) {
			$header[]= 'EAN';
		}
		if ($this->config->get('module_export_jan_status')) {
			$header[]= 'JAN';
		}
		if ($this->config->get('module_export_isbn_status')) {
			$header[]= 'ISBN';
		}
		if ($this->config->get('module_export_mpn_status')) {
			$header[]= 'MPN';
		}
		if ($this->config->get('module_export_location_status')) {
			$header[]= 'Location';
		}
		if ($this->config->get('module_export_shipping_status')) {
			$header[]= 'Shipping';
		}
		if ($this->config->get('module_export_points_status')) {
			$header[]= 'Points';
		}
		if ($this->config->get('module_export_weight_status')) {
			$header[]= 'Weight';
		}
		if ($this->config->get('module_export_weight_class_id_status')) {
			$header[]= 'Weight Class Id';
		}
		if ($this->config->get('module_export_weight_class_status')) {
			$header[]= 'Weight Class';
		}
		if ($this->config->get('module_export_length_status')) {
			$header[]= 'Length';
		}
		if ($this->config->get('module_export_width_status')) {
			$header[]= 'Width';
		}
		if ($this->config->get('module_export_height_status')) {
			$header[]= 'Height';
		}
		if ($this->config->get('module_export_length_class_id_status')) {
			$header[]= 'Length Class Id';
		}
		if ($this->config->get('module_export_length_class_status')) {
			$header[]= 'Length Class';
		}
		if ($this->config->get('module_export_subtract_status')) {
			$header[]= 'subtract';
		}
		if ($this->config->get('module_export_minimum_status')) {
			$header[]= 'Minimum';
		}
		if ($this->config->get('module_export_sort_order_status')) {
			$header[]= 'Sort Order';
		}
		if ($this->config->get('module_export_tag_status')) {
			$header[]= 'Tag';
		}
		if ($this->config->get('module_export_meta_title_status')) {
			$header[]= 'Meta Title';
		}
		if ($this->config->get('module_export_meta_description_status')) {
			$header[]= 'Meta Description';
		}
		if ($this->config->get('module_export_meta_keyword_status')) {
			$header[]= 'Meta Keyword';
		}
		if ($this->config->get('module_export_date_added_status')) {
			$header[]= 'Date Added';
		}
		if ($this->config->get('module_export_date_modified_status')) {
			$header[]= 'Date Modified';
		}
		if ($this->config->get('module_export_date_available_status')) {
			$header[]= 'Date Available';
		}
		if ($this->config->get('module_export_attribute_id_status')) {
			$header[]= 'Attribute Id';
		}
		if ($this->config->get('module_export_attribute_name_status')) {
			$header[]= 'Attribute Name';
		}
		if ($this->config->get('module_export_attribute_value_status')) {
		...
		foreach ($results as $result) {
			...
			if ($this->config->get('module_export_related_product_status')) {
				$export_data[]= $this->model_catalog_product->getRelatedProduct($result['product_id']);
			}
			if ($this->config->get('module_export_product_status_status')) {
				if ($result['status']==1) {
					$export_data[]= "Enable";
			...
			if ($this->config->get('module_export_category_status')) {
				$export_data[]= $this->model_catalog_product->getAllCategory($result['product_id']);
			}
			if ($this->config->get('module_export_tax_class_id_status')) {
	...
	public function exportProductImage(){
		...
		foreach ($results as $result) {
			...
			$export_data = array();
			$export_data[]= $result['product_id'];
			$export_data[]= $this->model_catalog_product->getProductImage($result['product_id']);
			fputcsv($file,$export_data);
		} 
